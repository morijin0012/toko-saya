...
class PHPWebViewClient(
    ...
    fun handlePHPRequest(
        ...
        // Only ever present on the JS-driven multipart file-upload flow
        // (see WebViewManager's injected submitMultipartForm) — a normal
        // request never sends this header. When present, a 3xx response is
        // returned to the caller AS-IS instead of being resolved
        // server-side, so fetch()'s own (real) redirect-follow takes over
        // and the JS layer can navigate the WebView to the correct final
        // URL via a real top-level navigation — restoring the full page
        // lifecycle (script re-injection, CSRF re-extraction) that a
        // spliced-in response body would otherwise skip. Every other
        // request path is completely unaffected.
        val skipRedirectFollow = headers.remove("X-NativePHP-Skip-Redirect-Follow") != null
        ...
        if (statusCode in 300..399) {
            ...
            if (!location.isNullOrEmpty()) {
                if (skipRedirectFollow) {
                    Log.d(TAG, "↪️ Passing raw redirect through (skip-follow requested): $location")
